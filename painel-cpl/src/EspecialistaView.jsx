...
// Coluna real do card: se está marcado como "Otimizado" manualmente, respeita
// isso (é decisão do especialista, protegida pelo congelamento de 3 dias).
// Fora isso, recalcula ao vivo comparando CPL atual x teto — não fica preso
// ao último Status gravado pelo n8n, que só roda a cada 3 dias.
function statusEfetivo(cliente) {
  const salvo = normalizeStatus(cliente.Status);
  if (salvo === 'otimizado') return 'otimizado';
  const teto = cliente['Custo por Resultado Máximo'];
  if (cliente.cpl !== null && cliente.cpl !== undefined && teto !== null && teto !== undefined) {
    return cliente.cpl > teto ? 'alto' : 'normal';
  }
  // sem CPL ao vivo ou sem teto definido pra comparar — usa o que já estava salvo
  return salvo;
}
...
function diasRestantesCongelamento(otimizadoEm) {
  if (!otimizadoEm) return null;
  const liberaEm = new Date(otimizadoEm).getTime() + 3 * 24 * 60 * 60 * 1000;
  const restanteMs = liberaEm - Date.now();
...
function Card({ cliente, columnKey, onOpen, onDragStart, onDragEnd }) {
  ...
  const pctOfMax =
    over && cliente['Custo por Resultado Máximo']
      ...
      : null;
  const diasRestantes = columnKey === 'otimizado' ? diasRestantesCongelamento(cliente.otimizado_em) : null;
  ...
  return (
    ...
    >
      ...
      {diasRestantes !== null && diasRestantes > 0 && (
        <span className="kcard-freeze-badge">🔒 travado por mais {diasRestantes}d</span>
      )}
...
function DetailModal({ cliente, onClose, onSave, onMove, moving }) {
  ...
  const current = statusEfetivo(cliente);
  const over = isOverBudget(cliente);
  ...
  const diasRestantes = current === 'otimizado' ? diasRestantesCongelamento(cliente.otimizado_em) : null;
  const travado = diasRestantes !== null && diasRestantes > 0;
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        {travado && (
          <div className="freeze-notice">
            🔒 Este cliente está travado em "Otimizados" — a automação não vai recalcular o status
            automaticamente pelos próximos {diasRestantes} dia{diasRestantes > 1 ? 's' : ''}. Você ainda pode
            mover manualmente se precisar.
          </div>
        )}
...
export default function EspecialistaView({ userEmail, onLogout }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // CPL "ao vivo" do card: pega o dia mais recente de metricas_diarias pra
      // esse cliente e calcula o CPL ponderado pelo investimento entre as
      // campanhas daquele dia (não usa mais historico_verificacoes, que não
      // recebe mais gravações do n8n).
      function cplDoDiaMaisRecente(chave) {
        const dias = historico7dByAdAccount[chave];
        if (!dias || dias.length === 0) return { cpl: null, data: null };
        const diaRecente = dias[0];
        let pesoTotal = 0;
        let somaPonderada = 0;
        for (const camp of diaRecente.campanhas) {
          if (camp.cpl === null) continue;
          const peso = camp.spend || 0;
          somaPonderada += camp.cpl * peso;
          pesoTotal += peso;
        }
        return { cpl: pesoTotal > 0 ? somaPonderada / pesoTotal : null, data: diaRecente.data };
      }

      const merged = clientesData.map((c) => {
        const { cpl: cplMetricas, data: dataMetricas } = cplDoDiaMaisRecente(c.ID);
        return {
          ...c,
          cpl: cplMetricas ?? c.cpl_atual ?? latestByClient[c.id]?.custo_por_resultado ?? null,
          ultima_verificacao: dataMetricas
            ? `${dataMetricas}T12:00:00`
            : latestByClient[c.id]?.data_verificacao ?? c.atualizado_em ?? null,
  ...
  const updateStatus = useCallback(async (id, dbStatusValue) => {
    ...
    // marca a data em que virou "Otimizado" (o n8n usa isso pra não sobrescrever
    // o status automaticamente nos 3 dias seguintes); ao sair de Otimizados manualmente,
    // limpa essa data já que o congelamento não faz mais sentido
    const payload = { Status: dbStatusValue, atualizado_em: new Date().toISOString() };
    if (dbStatusValue === 'Otimizado') {
      payload.otimizado_em = new Date().toISOString();
    } else {
      payload.otimizado_em = null;
    }
    ...
    try {
      ...
      setClientes((prev) => prev.map((c) => (c.id === id ? { ...c, Status: confirmedStatus, otimizado_em: rows[0].otimizado_em } : c)));
      setToast({ kind: 'ok', message: `${previous?.Cliente || 'Cliente'} movido para ${dbStatusValue}` });
      setSelected((prev) => (prev && prev.id === id ? { ...prev, Status: confirmedStatus, otimizado_em: rows[0].otimizado_em } : prev));
    } catch (e) {
  ...
  const byColumn = useMemo(() => {
    ...
    for (const c of filtrados) groups[statusEfetivo(c)].push(c);
