        ...
        {cliente.historico7d && cliente.historico7d.length > 0 && (
          <div className="hist-box">
            ...
            <div className="hist-table">
              ...
              {cliente.historico7d.map((d) => (
                <div className="hist-row" key={d.data}>
                  <span className="hist-date">{new Date(d.data + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}</span>
                  <span>{fmtMoney(d.spend)}</span>
                  <span>{fmtMoney(d.cpl)}</span>
                  <span>{d.ctr !== null ? `${d.ctr.toFixed(2)}%` : '—'}</span>
                  <span>{fmtMoney(d.cpm)}</span>
                  <span>{d.frequency !== null ? d.frequency.toFixed(2) : '—'}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    try {
      ...
      const [
        ...
        supabase
          ...
          .select('cliente_id,data,spend,ctr,cpm,frequency,custo_por_resultado')
      ...
      // Pode ter várias campanhas por cliente no mesmo dia — agrega por dia,
      // somando investimento e fazendo média ponderada do CPL pelo investimento.
      let porClienteDia = {};
      if (metricasData) {
        for (const row of metricasData) {
          const chave = row.cliente_id;
          if (!porClienteDia[chave]) porClienteDia[chave] = {};
          if (!porClienteDia[chave][row.data]) {
            porClienteDia[chave][row.data] = {
              data: row.data,
              spend: 0,
              cplSomaPonderada: 0,
              cplPeso: 0,
          ...
          const g = porClienteDia[chave][row.data];
          const spend = Number(row.spend) || 0;
          g.spend += spend;
          g.cplSomaPonderada += (Number(row.custo_por_resultado) || 0) * spend;
          g.cplPeso += spend;
          g.ctrSoma += Number(row.ctr) || 0;
          g.cpmSoma += Number(row.cpm) || 0;
          g.freqSoma += Number(row.frequency) || 0;
          g.n += 1;
        }
      }
      ...
      for (const chave of Object.keys(porClienteDia)) {
        const dias = Object.values(porClienteDia[chave])
          .map((g) => ({
            data: g.data,
            spend: g.spend,
            cpl: g.cplPeso > 0 ? g.cplSomaPonderada / g.cplPeso : null,
            ctr: g.n > 0 ? g.ctrSoma / g.n : null,
            cpm: g.n > 0 ? g.cpmSoma / g.n : null,
            frequency: g.n > 0 ? g.freqSoma / g.n : null,
          }))
  ...
  return (
    <div className="painel-root">
      <style>{`
        ...
        .hist-box-tag {
          font-family: var(--mono); font-size: 10px; font-weight: 700; color: var(--text-faint);
          letter-spacing: 0.06em; text-transform: uppercase; margin-bottom: 9px;
        }
        .hist-table { display: flex; flex-direction: column; gap: 2px; }
        .hist-row {
          display: grid;
          grid-template-columns: 1.1fr 1.3fr 1.3fr 1fr 1.2fr 0.9fr;
          gap: 4px;
          font-family: var(--mono);
          font-size: 11.5px;
          color: var(--text-dim);
        ...
        .hist-row:not(.hist-head):hover { background: rgba(255,255,255,0.03); }
        .hist-head {
          color: var(--text-faint); font-size: 9.5px; text-transform: uppercase; letter-spacing: 0.03em;
          border-bottom: 1px solid rgba(255,255,255,0.06); padding-bottom: 6px; margin-bottom: 2px;
        }
