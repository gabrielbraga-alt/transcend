    ...
    >
      ...
      <div className="kcard-head">
        ...
        <div className="kcard-icons">
          {cliente.sugestaoIA ? (
            <span className={`kcard-ia-dot ia-${(cliente.sugestaoIA.status_ia || '').toLowerCase()}`} title="Tem sugestão da IA">IA</span>
          ) : null}
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        {cliente.sugestaoIA && (
          <div className={`ia-box ia-box-${(cliente.sugestaoIA.status_ia || '').toLowerCase()}`}>
            <div className="ia-box-head">
              <span className="ia-box-tag">✨ sugestão da IA</span>
              <span className="ia-box-status">{cliente.sugestaoIA.status_ia}</span>
            </div>
            {cliente.sugestaoIA.campaign_name && (
              <div className="ia-box-campaign">{cliente.sugestaoIA.campaign_name}</div>
            )}
            <div className="ia-box-resumo">{cliente.sugestaoIA.resumo}</div>
            {cliente.sugestaoIA.sugestao && (
              <div className="ia-box-sugestao">
                <span className="ia-box-sugestao-label">recomendação:</span> {cliente.sugestaoIA.sugestao}
              </div>
            )}
            <div className="ia-box-time">{timeAgo(cliente.sugestaoIA.criado_em)}</div>
          </div>
        )}
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [
        ...
        supabase
          .from('resumo_ia')
          .select('cliente_id,cliente,status_ia,resumo,sugestao,campaign_name,criado_em')
      ...
      // resumo_ia.cliente_id é o ID da conta de anúncio (bate com clientes.ID, não com clientes.id/uuid)
  ...
  return (
    <div className="painel-root">
      <style>{`
        ...
        .kcard-ia-dot {
          font-family: var(--mono); font-size: 8.5px; font-weight: 700; letter-spacing: 0.02em;
          padding: 1px 5px; border-radius: 4px; line-height: 1.5;
        }
        .kcard-ia-dot.ia-crítico { background: #3e1a16; color: var(--red); }
        .kcard-ia-dot.ia-atenção { background: var(--amber-dim); color: var(--amber); }
        .kcard-ia-dot.ia-ok, .kcard-ia-dot.ia-saudável { background: var(--green-dim); color: var(--green); }
        ...
        .ia-box {
          ...
          border: 1px solid rgba(79,179,240,0.22);
          border-radius: 11px;
          padding: 13px 14px;
          margin-bottom: 18px;
        }
        .ia-box-crítico { border-color: rgba(234,107,87,0.3); }
        .ia-box-atenção { border-color: rgba(236,171,79,0.3); }
        .ia-box-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
        .ia-box-tag {
          font-family: var(--mono); font-size: 10.5px; font-weight: 700; color: var(--blue);
          letter-spacing: 0.02em;
        }
        .ia-box-status {
          font-family: var(--mono); font-size: 10px; font-weight: 700; text-transform: uppercase;
          padding: 2px 8px; border-radius: 5px; background: var(--blue-dim); color: var(--blue);
        }
        .ia-box-crítico .ia-box-status { background: #3e1a16; color: var(--red); }
        .ia-box-atenção .ia-box-status { background: var(--amber-dim); color: var(--amber); }
        .ia-box-campaign {
          font-family: var(--mono); font-size: 10.5px; color: var(--text-faint);
          margin-bottom: 8px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
        }
        .ia-box-resumo { font-size: 13px; color: var(--text); line-height: 1.5; margin-bottom: 8px; }
        .ia-box-sugestao {
          font-size: 12.5px; color: var(--text-dim); line-height: 1.5;
          background: rgba(255,255,255,0.03); border-radius: 8px; padding: 9px 11px; margin-bottom: 8px;
        }
        .ia-box-sugestao-label { color: var(--blue); font-weight: 600; }
        .ia-box-time { font-family: var(--mono); font-size: 10px; color: var(--text-faint); }
