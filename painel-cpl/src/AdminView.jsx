...
// Mesma lógica do Kanban: respeita "Otimizado" (manual, protegido pelo
// congelamento), mas recalcula Alto/Normal ao vivo comparando CPL x teto,
// em vez de confiar só no último Status gravado pelo n8n.
...
export default function AdminView({ userEmail, onLogout }) {
    ...
    try {
      ...
      const [
        ...
        supabase
          ...
          .select('cliente_id,data,spend,custo_por_resultado')
      ...
      // Pega o dia mais recente de metricas_diarias por cliente e calcula o
      // CPL ponderado pelo investimento entre as campanhas daquele dia.
      let diaMaisRecentePorCliente = {};
      if (metricasData) {
        for (const row of metricasData) {
          const chave = row.cliente_id;
          if (!diaMaisRecentePorCliente[chave]) {
            diaMaisRecentePorCliente[chave] = { data: row.data, somaPonderada: 0, pesoTotal: 0 };
          }
          const atual = diaMaisRecentePorCliente[chave];
          if (row.data !== atual.data) continue; // já passou do dia mais recente (dados vêm ordenados desc)
          const peso = Number(row.spend) || 0;
          if (row.custo_por_resultado !== null) {
            atual.somaPonderada += Number(row.custo_por_resultado) * peso;
      ...
      const merged = clientesData.map((c) => {
        const agregado = diaMaisRecentePorCliente[c.ID];
        const cplMetricas = agregado && agregado.pesoTotal > 0 ? agregado.somaPonderada / agregado.pesoTotal : null;
        return {
          ...c,
          cpl: cplMetricas ?? c.cpl_atual ?? latestByClient[c.id]?.custo_por_resultado ?? null,
          ultima_verificacao: agregado?.data
            ? `${agregado.data}T12:00:00`
  ...
  const filtrados = useMemo(() => {
    let list = clientes.filter((c) => {
      if (statusFiltro !== 'todos' && statusEfetivo(c) !== statusFiltro) return false;
  ...
  const counts = useMemo(() => {
    ...
    for (const cli of clientes) c[statusEfetivo(cli)]++;
    return c;
  }, [clientes]);
          ...
          <>
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {filtrados.length === 0 ? (
                    <tr><td colSpan={7} className="empty-row">nenhum cliente encontrado</td></tr>
                  ) : (
                    filtrados.map((c) => {
