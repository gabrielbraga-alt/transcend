...
-- Tabela com as sugestões geradas por IA (uma linha por verificação/campanha)
alter table resumo_ia enable row level security;

create policy "autenticados leem resumo_ia"
  on resumo_ia for select
  using (auth.role() = 'authenticated');
